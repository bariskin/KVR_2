...
IPAddress ip(10,0,0,15);                            //статический IP для рубильника
...
{ 
  ...
  pinMode(pin_in_1,  INPUT);                 // подтяжка пина  к плюсу 
  pinMode(pin_in_2,  INPUT);                 // подтяжка пина  к плюсу
...
void loop()
{
 ...
 if (packetSize)
 {
   ...
   { 
    ...
    { 
     if ((incomingPacket[7] == 'O') && (incomingPacket[8] == 'F') && (incomingPacket[9] == 'F')) { input_1_flag = 1; Serial.print("Board status: PIN_1 = DISABLED\r\n"); }
    ...
    { 
     if ((incomingPacket[7] == 'O') && (incomingPacket[8] == 'F') && (incomingPacket[9] == 'F')) { input_2_flag = 1; Serial.print("Board status: PIN_2 = DISABLED\r\n"); }
 ...
 // --------------------------------------- DIGITAL INPUT ----------------------------- //
 if ((input_1_flag == 1) && (TRIGGER1_flag == 0) && (digitalRead(pin_in_1) == LOW))
 {
 ...
 if ((input_1_flag == 1) && (TRIGGER1_flag == 1) && (digitalRead(pin_in_1) == HIGH))
 {
  Udp.beginPacket(ip_sh1, port); 
